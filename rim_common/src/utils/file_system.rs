...
pub fn remove<P: AsRef<Path>>(src: P) -> Result<()> {
    if src.as_ref().is_file() || src.as_ref().is_symlink() {
        fs::remove_file(&src)
...
///
/// # Error
/// Return error if [`fs::hard_link`] failes, meaning that the `original`
/// is likely a directory or doesn't exists at all.
